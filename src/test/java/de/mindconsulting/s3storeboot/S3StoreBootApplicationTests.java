...
import com.ytfs.client.batch.MakeFile;
...
//	}
	...
	@Test
	public void writeAyncFileMeta() throws IOException, JAXBException {
		String key = "testtt";
		String bucketName = "bucket-test-01";
		String path = "D:/s3cache/sync";
		String pathXML = path+"/"+bucketName+"/"+key + "_meta.xml";
		Path filePath = Paths.get(pathXML);
		String directories = path+"/"+bucketName;
...
//		header.put("meta",null);


		AyncFileMeta ayncFileMeta = new AyncFileMeta();
		ayncFileMeta.setBucketname(bucketName);
		ayncFileMeta.setKey(key);
		ayncFileMeta.setPath(filePath.toString());

		try (OutputStream out = Files.newOutputStream(filePath)) {
			Marshaller m = jaxbContext.createMarshaller();
			m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
			m.marshal(ayncFileMeta, out);
		}
...
//		}
    ...
    public void uploadTest() throws Exception{
		YTClient client = YTClientMgr.newInstance("shanpaohello","5JS1Wsr5RZvbBqxnARoV2PSzGnnfpRuAxSBJkLDw98yYNnsxgU6");
		MakeFile makeFile = new MakeFile("D:\\20200309-new\\s3cache\\test", "ssssssss");
		String uuid = UUID.randomUUID().toString();
		long len = makeFile.makeFile(false);
		Map<String,String> header = new HashMap<>();
		header.put("contentLength",len+"");
...
//		LOG.info("Make file ok,length " + len / 1024L / 1024L + "M,take times " + (System.currentTimeMillis() - l) + " ms");
		...
		UploadObject obj = client.createUploadObject(makeFile.getFilePath());

		obj.upload();

		header.put("ETag", Hex.encodeHexString(obj.getVHW()));
		byte[] bs = SerializationUtil.serializeMap(header);
		UploadObjectEndReqV2 req = new UploadObjectEndReqV2();
		req.setVHW(obj.getVHW());
		req.setVNU(obj.getVNU());
		req.fill(client.getUserId(), client.getKeyNumber(), client.getPrivateKey());
		P2PUtils.requestBPU(req, client.getSuperNode(), obj.getVNU().toString(), UserConfig.SN_RETRYTIMES);
		client.createObjectAccessor().createObject("ssssssss", uuid, obj.getVNU(), bs);
    }
